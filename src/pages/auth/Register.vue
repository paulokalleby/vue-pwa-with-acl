  ...
  <v-form @submit.prevent="handlerRegister">
    <v-row class="pa-xl-12 pa-lg-8 pa-6">
      ...
      <v-col cols="12" class="text-start pt-0 pb-2">
        <v-text-field v-model="data.name" prepend-inner-icon="lucide:User"
          :error-messages="v$.name.$errors.map((e) => e.$message)" @input="v$.name.$touch" @blur="v$.name.$touch"
          label="Nome" />
      </v-col>
      ...
      <v-col cols="12" class="pt-0 pb-1">
        <v-btn type="submit" color="primary" flat block class="mb-3 float-start" :loading="store.creating"
          text="Registrar">
        </v-btn>
      </v-col>
